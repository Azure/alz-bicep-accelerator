    ...
    contains(parPolicyAssignmentParameterOverrides, policyAssignment.name) ? {
      ...
      properties: union(
        ...
        parPolicyAssignmentParameterOverrides[policyAssignment.name].?scope != null ? {
          ...
          policyDefinitionId: replace(policyAssignment.properties.policyDefinitionId, '/managementGroups/alz/', '/managementGroups/${landingZonesCorpConfig.?managementGroupName ?? 'landingzones-corp'}/')
        } : {
          scope: '/providers/Microsoft.Management/managementGroups/${landingZonesCorpConfig.?managementGroupName ?? 'landingzones-corp'}'
          policyDefinitionId: replace(policyAssignment.properties.policyDefinitionId, '/managementGroups/alz/', '/managementGroups/${landingZonesCorpConfig.?managementGroupName ?? 'landingzones-corp'}/')
        },
...
module landingZonesCorp 'br/public:avm/ptn/alz/empty:0.3.1' = {
  params: {
    ...
    managementGroupName: landingZonesCorpConfig.?managementGroupName ?? 'landingzones-corp'
